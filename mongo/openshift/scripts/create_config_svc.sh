#!/bin/bash
oc apply -f "../k8s-config/config-deployment.yaml"

time_count=0
echo "============== Creating config pods (~1 min) =============="
while [[ $(oc get pods -l name="configdb" -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}') != "True True True" ]]; do      if [[ $time_count -eq 24 ]]; then
